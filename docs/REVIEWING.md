...
| **Semantic lints** | Issuer is registered, network tier is atomic and allow-listed, rates aren't fraction-encoded, dates aren't in the future, fee currency matches the market, `image.url` is not a stock photo / social banner / OG image / award badge, two Card Ids do not share a URL unless they are in `SHARED_ART_FAMILIES`, and `image.local_path` / `image.history[].local_path` files exist in the checkout. | That the rate/fee/date is the one on the bank's page, or that a unique `image.url` actually depicts this product. |
